...
async def find_canvas_video_frame(page: Page):
    for _ in range(10):
...
async def extract_video_url(page: Page) -> str:
    shared_state = {"video_url": None}
    await register_cdp_video_sniffer(page, shared_state)

    video_frame = await find_canvas_video_frame(page)
    if not video_frame:
        return None
